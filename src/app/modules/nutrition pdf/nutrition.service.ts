...
import PetNutritionModel from "./nutrition.model";

export const generatePetNutritionPDFService = async (
  petType: string,
  age: number,
  weight: number
): Promise<Buffer> => {
  ...
  try {
    ...
    const nutritionData = await PetNutritionModel.findOne({
      petType,
      ageRange,
    });
    ...
    page.drawText("Pet Nutrition Chart", {
      ...
      y: 770,
      size: 30,
      color: rgb(0.2, 0.5, 0.8), // Blue color
      font: helveticaBold,
    });
...
function getAgeRange(petType: string, age: number): string {
...
function calculateCalories(
  ...
  switch (petType.toLowerCase()) {
    case "dog":
      if (age < 1) baseCalories *= 1.2; // Puppies need more calories
      if (age > 7) baseCalories *= 0.8; // Senior dogs need fewer calories
      break;
    case "cat":
      if (age < 1) baseCalories *= 1.3; // Kittens need more calories
      if (age > 7) baseCalories *= 0.9; // Senior cats need slightly fewer calories
      break;
    case "rabbit":
      if (age < 1) baseCalories *= 1.2; // Young rabbits need more calories
      if (age > 6) baseCalories *= 0.9; // Senior rabbits need fewer calories
      break;
    default:
      throw new Error("Unsupported pet type");
  }
